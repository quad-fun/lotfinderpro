#!/usr/bin/env python3
"""
NYC PLUTO Data Loader for LotFinder Pro - FIXED VERSION
...
- Run script: python pluto_loader_fixed.py [--incremental] [--borough BOROUGH]
"""
...
# Add this at the top of pluto_loader_fixed.py
...
def fetch_pluto_data(offset=0, limit=BATCH_SIZE, where_clause=None):
    ...
    try:
        ...
        headers = {
            'User-Agent': 'LotFinderPro/1.0 (Data Loader for Real Estate Analysis)',
...
def test_api_connection():
    """Test the connection to the PLUTO API."""
    print("Testing API connection...")
    try:
        # Try to get just one record to test the connection
        url = f"{PLUTO_API_BASE_URL}?$limit=1"
        headers = {
            'User-Agent': 'LotFinderPro/1.0 (Data Loader for Real Estate Analysis)',
            'Accept': 'application/json'
        }
...
def main():
    ...
    parser.add_argument('--test-only', action='store_true', help='Test the API connection and exit')
    ...
    if args.test_only:
        if test_api_connection():
            print("API test successful. Exiting.")
            sys.exit(0)
        else:
